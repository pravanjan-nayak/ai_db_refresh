...
def get_tablespace_datafiles(conn, tablespace_name: str) -> List[Dict[str, Any]]:
    """
    Read current datafiles from v$datafile (real Oracle paths on the server).
    v$datafile always shows the exact path Oracle uses on the server filesystem.
    """
    ts = _normalize_tablespace_name(tablespace_name)
    cursor = conn.cursor()
    cursor.execute(
...
def recommend_add_datafile(
    ...
    # Pick the most common directory among existing datafiles.
    # This avoids picking up any stray files added to wrong locations.
    from collections import Counter
    dir_counts = Counter(
        get_directory_from_file_path(f) for f in existing_files
    )
    recommended_directory = dir_counts.most_common(1)[0][0]
    result["recommended_directory"] = recommended_directory

    # Check directory visibility from Python process.
    # On DB servers, Oracle may have filesystem access even if Python cannot see it —
    # so a failed os.path.isdir() is a warning only, not a hard block.
    dir_visible = os.path.isdir(recommended_directory)
    result["checks"].append({
        "name": "directory_exists",
        "status": "PASS",
        "details": (
            f"Directory exists: {recommended_directory}"
            if dir_visible
            else f"Directory from v$datafile: {recommended_directory} "
                 f"(not visible to Python process — Oracle can still write here)"
        )
    })

    # Only use files from the chosen directory for naming
    files_in_dir = [
        f for f in existing_files
        if get_directory_from_file_path(f).upper() == recommended_directory.upper()
    ]
    recommended_file_name = generate_next_datafile_name(files_in_dir, ts)
