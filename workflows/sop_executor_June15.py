...
def run_sop_action(action_template: str, selected_row: dict, sop_name: str, approved_by: str = "DBA") -> dict:
    """
    Fires the action SQL (e.g. ALTER SYSTEM KILL SESSION) after DBA approval.
    Only called when the DBA clicks 'Approve & Execute' in the UI.
    """
    action_sql = build_action_sql(action_template, selected_row)
    ...
    try:
        ...
        return {
            ...
            "message": f"Action executed successfully on Oracle: {action_sql}",
            "error": None
        }
