...
if ask_btn and question.strip():
    ...
    # -------------------------------------------------
    # MCP path — SOP-aware routing
    # -------------------------------------------------
    elif query_mode == "MCP Server":

        # Save question so the SOP display section can re-use it
        st.session_state["last_question"] = question.strip()

        # Check if a SOP matches before calling the general query tool
        from workflows.sop_matcher import match_sop
        sop_match = match_sop(question.strip())

        if sop_match.get("matched"):
            # SOP matched — show markdown + "Run Live Diagnostic" button.
            # The actual SQL only runs when DBA clicks that button.
            result = {
                "success": True,
                "route": "sop",
            ...
            # Clear any previous diagnostic from a different SOP
            st.session_state["sop_diagnostic_result"] = None
        else:
            # No SOP matched — use the normal query tool
    ...
    if result.get("success"):
        ...
        if not result.get("direct_mode"):
            ...
            if result.get("route") == "change_request":
                ...
                # Show execution result from session state,
                # or fall back to the result embedded in the change request record
                exec_result = st.session_state.get("last_change_execution_result")

                if not exec_result:
                    # Try to build a display result from the stored record
                    request_id = result.get("request_id", "")
                    if request_id:
            ...
            # -------------------------------------------------
            # SOP Route Display — Live Diagnostic + Approval
            # -------------------------------------------------
                ...
                if diag and diag.get("sop_name") == current_task:
                    ...
                    if diag.get("success") and diag.get("rows"):
                        ...
                        # ── PHASE B: Action needed → row picker + approval ──
                        if diag.get("action_required") and diag.get("action_template"):
                            ...
                            # Use the most meaningful columns for the row label
                            label_cols = [c for c in columns if c in (
                                "sid", "serial#", "blocking_sid", "blocking_serial",
                                "username", "blocking_user", "status", "machine"
                            )]
                            if not label_cols:
                            ...
                            # Show the action SQL before approval
                            ...
                            with col_approve:
                                if st.button("✅ Approve & Execute", key="sop_approve_btn", type="primary"):
                                    with st.spinner("Executing Database action on Oracle..."):
                                        # 1. We call the MCP tool and save it directly into 'execution_response'
                                        ...
                                        # Save it into the session state variable that our UI block watches
                                        st.session_state["execution_result"] = exec_result
        
                                        # Force Streamlit to immediately rerun and draw the status layout
                                        st.rerun()
                                    if exec_result.get("success"):
                                        st.success(exec_result.get("message"))
                                        st.code(exec_result.get("action_sql", ""), language="sql")
                                    else:
                                        st.error(exec_result.get("message"))
                            ...
                            # Diagnostic-only SOP (e.g. tablespace monitoring)
                    ...
                    # ── Diagnostic not yet run — show SOP markdown + button ──
                    sop_content = result.get("sop_content", "")
                    if sop_content:
                        # Wrap it in a clean, interactive expander block:
...
if clear_results:
    ...
    st.session_state["last_change_execution_result"] = None
    # ===== NEW BLOCK END =====
    # ===== NEW BLOCK START: Clear SOP state =====
    st.session_state["sop_diagnostic_result"] = None
    st.session_state["last_question"] = ""
    # ===== NEW BLOCK END =====
...
# ==================================================
# SHOW EXECUTION RESULT (MERGED FOR SOPs & REFRESH)
# ==================================================
if st.session_state["execution_result"]:
    ...
    if result.get("success"):
        ...
        # ── NEW SOP VISUALIZATIONS ──
        if "action_sql" in result:
            st.info(f"**Executed SQL Command:** `{result['action_sql']}`")
