                    ...
                    if diag and diag.get("sop_name") == current_task:
                        ...
                        if st.button("▶ Run Live Diagnostic on Oracle", key="sop_run_diagnostic", type="primary"):
                            with st.spinner("Running diagnostic query on Oracle..."):
                                # 1. Extract the question string safely
                                current_question = st.session_state.get("last_question", "")
                                
                                # 2. Use regex to grab the database name dynamically
                                import re
                                match = re.search(r"(?:convert non-cdb to pdb|migrate)\s+(\w+)", current_question, re.IGNORECASE)
                                extracted_pdb = match.group(1).upper() if match else "UNKNOWN"
                                
                                # 3. Send both the question and the extracted pdb_name parameter
                                diag_result = call_mcp("sop_diagnose", {
                                    "question": current_question,
                                    "pdb_name": extracted_pdb
                                })
